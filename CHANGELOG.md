...
- **修复停止后误报"处理完成"** — done 事件新增 `stopped` 标志，用户停止时进度面板显示"总进度：已停止（部分完成）"，日志与系统通知改为"处理已停止"文案，不再把进度条强制置 100%。
- **修复单文件失败中断整批** — 串行与并行流水线改为逐文件容错：单个文件转写/翻译失败只记 ERROR 日志并跳过继续，全部失败才报整体错误；done 消息带失败文件数。
- **修复断点续翻串视频** — `translate_state.json` 与续翻源字幕候选均按 `safe_stem` 前缀过滤，不再拿别的视频残留的状态/字幕续翻本视频；完成标记跳过也不再被别的视频的 state 卡住。
- **修复 `.partial.srt` 被当成品** — `find_existing_subtitle` 与续翻候选排除 `partial` 断点文件，崩溃遗留的半截字幕不再被直接翻译。
- **修复同名不同格式输出互相覆盖** — `_run` 开头检测同目录同 stem 的视频/音频文件（如 `a.mp4` 与 `a.mkv`），直接报错中止并提示用户重命名/移走，杜绝 partial/srt/mkv 输出互相覆盖与并行互删。
- **修复转写失败显存泄漏** — 阶段 1 转写失败不再直接 return，所有退出路径统一释放 Whisper 显存。
- **修复本地服务并发互杀** — `ensure_running` 仅"实际拉起者"超时有权终止服务并重置状态，并发等待者超时只返回失败，不再反复杀-起 llama-server 导致全部翻译失败。
- **修复关窗强杀外部服务** — 关闭窗口只停本会话拉起的 llama-server，不再 `taskkill` 用户手动启动的外部服务；关窗先 `worker.stop()` 再释放显存，避免与转写线程竞争。
- **修复 GBK 字幕读取** — `parse_srt` 与预览加载改用编码自动探测（utf-8-sig → gbk → latin-1），Windows 常见 GBK/ANSI 中文字幕不再中止整批翻译或预览空白。
- **修复 ETA 不显示** — 进度面板底部"已用/剩余/预计"布局从未挂到主布局，补 `addLayout(bot)` 一行修复。
- **修复 0 值配置崩溃** — `checkpoint_interval`/`batch_size` 读取处钳位到至少 1，配置为 0 不再 `% 0` / `range(0, len, 0)` 崩溃。
