...
| **🔗 下载器联动** | MissAV 下载器每批结束后可一键将成功视频导入当前窗口的待处理队列 |
...
> **后台运行**：点窗口关闭按钮会把程序隐藏到 Windows 右下角通知区域，正在运行的任务继续处理。左键单击图标、右键选择「打开窗口」，或再次运行快捷方式，均可恢复原窗口；再次运行不会创建第二个常驻进程。右键选择「彻底退出」才会关闭程序，任务仍在运行时会先确认。隐藏期间如需确认嵌入，会收到系统通知；打开窗口后再处理。
